...
AckermannToVesc::AckermannToVesc(ros::NodeHandle nh, ros::NodeHandle private_nh)
{
  ...
  if (!getRequiredParam(nh, "steering_angle_to_servo_offset", steering_to_servo_offset_))
    return;
  if (!getRequiredParam(nh, "steering_hysteresis_offset_right", steering_hysteresis_offset_right))
    return;
  if (!getRequiredParam(nh, "steering_hysteresis_offset_left", steering_hysteresis_offset_left))
    return;
...
{
  _Float64 cmd_angle_(cmd->drive.steering_angle);
  ...
  if (cmd_angle_==0)
  {
    if (past_turn_==1)
    {
      servo_msg->data = steering_to_servo_offset_ + steering_hysteresis_offset_left;
    }
    else if (past_turn_==-1)
    {
      servo_msg->data = steering_to_servo_offset_ - steering_hysteresis_offset_right;
    }
    else
    {
  ...
  {
    servo_msg->data = steering_to_servo_gain_ * cmd_angle_ + steering_to_servo_offset_;
    if (cmd_angle_>0)
    {
      past_turn_=1;
    }
