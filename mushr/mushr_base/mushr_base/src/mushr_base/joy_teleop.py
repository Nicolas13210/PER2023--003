...
class JoyTeleop:
    ...
    def joy_callback(self, data):
        try:
            for c in self.command_list:
                if self.match_command(c, data.buttons):
                    # data.buttons[0] pour carré
                    # data.buttons[1] pour croix
                    # data.buttons[2] pour rond
                    # data.buttons[3] pour triangle
                    if data.buttons[1] == 1:
                        if self.X_WAS_PRESSED == False:
                            self.X_WAS_PRESSED = True
                            if self.CAR_STOP == False:
                                self.publish_mqtt_message("true", "cars/emergencystop")
                                self.CAR_STOP = True
                            else:
                                self.publish_mqtt_message("false", "cars/emergencystop")
                                self.CAR_STOP = False
                    elif data.buttons[1] == 0:
                        self.X_WAS_PRESSED = False

                    if data.buttons[0] == 1:
                        self.publish_mqtt_message("1", "car2/model")

                    if data.buttons[2] == 1:
                        self.publish_mqtt_message("2", "car2/model")

                    if data.buttons[3] == 1:
                        self.publish_mqtt_message("3", "car2/model")
    ...
    def publish_mqtt_message(self, message, topic):
        mqtt_publish.single(topic, message, hostname="192.168.18.9", port=2883)
