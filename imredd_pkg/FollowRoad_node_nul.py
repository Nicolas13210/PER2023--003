...
# from model import get_mobilenet_2
...
#         # self.model = get_mobilenet_2()
#         self.model.load_state_dict(torch.load('/root/catkin_ws/src/imredd_pkg/models/kenza/ckpt_10.ckpt'))
...
#     #     speed, steering = self.model(self.preprocess(img_rgb)).detach().float().cpu().numpy().flatten()
...
#             # steering = steering_label +float(0.11)
...
class follow_road:
    def __init__(self):
        ...
        self.model.load_state_dict(torch.load(
            '/root/catkin_ws/src/imredd_pkg/ckpt_27.ckpt', map_location=torch.device('cpu')))
        ...
        self.dico = {'0': float(0.7), '1': float(0.4), '2': float(0.2), '3': float(
            0.0), '4': float(-0.2), '5': float(-0.4), '6': float(-0.6), '7': float(-0.7)}
        self._run()
    ...
    def callback(self, img, args):
        ...
        if self.img_rgb is not None:
            ...
            steering_label=output.item()
            # steering_label =torch.unsqueeze(output, dim=0).item() 
            # steering_label = torch.unsqueeze(steering_label, dim=0).item() #!
            # steering = self.dico[str(steering_label)]+float(0.11)
            # steering = self.dico[str(steering_label)]    #!
            speed = self.speed
        ...
        commande.drive.speed = speed * self.speed_factor
        #commande.drive.steering_angle = steering
        commande.drive.steering_angle = steering_label
        # commande.drive.steering_angle = output
        self.pub.publish(commande)
        self.rate.sleep()
