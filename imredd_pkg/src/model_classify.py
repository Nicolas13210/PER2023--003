...
class Model(nn.Module):
    def __init__(self):
        ...
        # Change the input size to reflect the modified output size
...
class Model_classif_1(nn.Module):
    def __init__(self):
        ...
        self.dense3 = torch.nn.Linear(128, 14, bias=True).cuda()
        ...
        # self.deepwise_conv1 = torch.nn.Conv2d(
        #     3, 3, kernel_size=3, padding='same', groups=3)
        # self.pointwise_conv2 = torch.nn.Conv2d(
        #     3, 64, kernel_size=1, padding='same')
        # self.bn1 = torch.nn.BatchNorm2d(3)
        # self.bn2 = torch.nn.BatchNorm2d(64)

        # self.deepwise_conv3 = torch.nn.Conv2d(
        #     64, 64, kernel_size=3, padding='same', groups=64)
        # self.pointwise_conv4 = torch.nn.Conv2d(
        #     64, 128, kernel_size=1, padding='same')
        # # self.bn3 = torch.nn.BatchNorm2d(64)
        # self.bn4 = torch.nn.BatchNorm2d(128)

        # self.deepwise_conv5 = torch.nn.Conv2d(
        #     128, 128, kernel_size=3, padding='same', groups=128)
        # self.pointwise_conv6 = torch.nn.Conv2d(
        #     128, 256, kernel_size=1, padding='same')
        # # self.bn5 = torch.nn.BatchNorm2d(128)
        # self.bn6 = torch.nn.BatchNorm2d(256)

        # self.deepwise_conv7 = torch.nn.Conv2d(
        #     256, 256, kernel_size=3, padding='same', groups=256)
        # self.pointwise_conv8 = torch.nn.Conv2d(
        #     256, 256, kernel_size=1, padding='same')
        ...
        # self.dense3 = torch.nn.Linear(128, 14, bias=True)
...
class Model_classif_lstm_1(nn.Module):
    def __init__(self):
        ...
        self.deepwise_conv1 = torch.nn.Conv3d(
            3, 3, kernel_size=(1, 3, 3), padding='same', groups=3).cuda()
        self.pointwise_conv2 = torch.nn.Conv3d(
            3, 64, kernel_size=1, padding='same').cuda()
        self.bn1 = torch.nn.BatchNorm3d(3).cuda()
        self.bn2 = torch.nn.BatchNorm3d(64).cuda()

        self.deepwise_conv3 = torch.nn.Conv3d(
            64, 64, kernel_size=(1, 3, 3), padding='same', groups=64).cuda()
        self.pointwise_conv4 = torch.nn.Conv3d(
            64, 128, kernel_size=1, padding='same').cuda()
        self.bn3 = torch.nn.BatchNorm3d(64).cuda()
        self.bn4 = torch.nn.BatchNorm3d(128).cuda()

        self.deepwise_conv5 = torch.nn.Conv3d(
            128, 128, kernel_size=(1, 3, 3), padding='same', groups=128).cuda()
        self.pointwise_conv6 = torch.nn.Conv3d(
            128, 256, kernel_size=1, padding='same').cuda()
        self.bn5 = torch.nn.BatchNorm3d(128).cuda()
        self.bn6 = torch.nn.BatchNorm3d(256).cuda()

        self.deepwise_conv7 = torch.nn.Conv3d(
            256, 256, kernel_size=(1, 3, 3), padding='same', groups=256).cuda()
        self.pointwise_conv8 = torch.nn.Conv3d(
            256, 256, kernel_size=1, padding='same').cuda()
        self.bn7 = torch.nn.BatchNorm3d(256).cuda()
        self.bn8 = torch.nn.BatchNorm3d(256).cuda()

        self.deepwise_conv9 = torch.nn.Conv3d(
            256, 256, kernel_size=(1, 3, 3), padding='same', groups=256).cuda()
        self.pointwise_conv10 = torch.nn.Conv3d(
            256, 256, kernel_size=1, padding='same').cuda()
        self.bn9 = torch.nn.BatchNorm3d(256).cuda()
        self.bn10 = torch.nn.BatchNorm3d(256).cuda()

        self.deepwise_conv11 = torch.nn.Conv3d(
            256, 256, kernel_size=(1, 3, 3), padding='same', groups=256).cuda()
        self.pointwise_conv12 = torch.nn.Conv3d(
            256, 256, kernel_size=1, padding='same').cuda()
        ...
        self.maxpooling = torch.nn.MaxPool3d(
            (1, 2, 2), stride=(1, 2, 2)).cuda()
        self.maxpooling_2 = torch.nn.MaxPool3d(
            (1, 1, 2), stride=(1, 1, 2)).cuda()
        self.relu = torch.nn.ReLU().cuda()
...
class Model_classif_2(nn.Module):
    def __init__(self):
        ...
        self.deepwise_conv1 = torch.nn.Conv2d(
            3, 3, kernel_size=3, padding='same', groups=3).cuda()
        self.pointwise_conv2 = torch.nn.Conv2d(
            3, 64, kernel_size=1, padding='same').cuda()
        self.bn1 = torch.nn.BatchNorm2d(3).cuda()
        self.bn2 = torch.nn.BatchNorm2d(64).cuda()
        ...
        self.deepwise_conv3 = torch.nn.Conv2d(
            64, 64, kernel_size=3, padding='same', groups=64).cuda()
        self.pointwise_conv4 = torch.nn.Conv2d(
            64, 128, kernel_size=1, padding='same').cuda()
        self.bn3 = torch.nn.BatchNorm2d(64).cuda()
        self.bn4 = torch.nn.BatchNorm2d(128).cuda()
        ...
        self.deepwise_conv5 = torch.nn.Conv2d(
            128, 128, kernel_size=3, padding='same', groups=128).cuda()
        self.pointwise_conv6 = torch.nn.Conv2d(
            128, 256, kernel_size=1, padding='same').cuda()
        self.bn5 = torch.nn.BatchNorm2d(128).cuda()
        self.bn6 = torch.nn.BatchNorm2d(256).cuda()
        ...
        self.deepwise_conv7 = torch.nn.Conv2d(
            256, 256, kernel_size=3, padding='same', groups=256).cuda()
        self.pointwise_conv8 = torch.nn.Conv2d(
            256, 256, kernel_size=1, padding='same').cuda()
        self.bn7 = torch.nn.BatchNorm2d(256).cuda()
        self.bn8 = torch.nn.BatchNorm2d(256).cuda()

        self.deepwise_conv9 = torch.nn.Conv2d(
            256, 256, kernel_size=3, padding='same', groups=256).cuda()
        self.pointwise_conv10 = torch.nn.Conv2d(
            256, 256, kernel_size=1, padding='same').cuda()
        self.bn9 = torch.nn.BatchNorm2d(256).cuda()
        self.bn10 = torch.nn.BatchNorm2d(256).cuda()
